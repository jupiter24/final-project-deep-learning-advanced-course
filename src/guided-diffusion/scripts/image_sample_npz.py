...
def main():
    ...
    while num_current_samples < args.num_samples:
        ...
        with th.no_grad():
            ...
            if args.condition_size == 256:
                feat = classification_model.head(feat)
        ...
        orig_images.extend([sample.unsqueeze(0).cpu().numpy() for sample in batch])

        sample = ((sample[0:1] + 1) * 127.5).clamp(0, 255).to(th.uint8)
